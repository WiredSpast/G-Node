...
export class HPacket {
    ...
    readShort(index) {
        ...
        if(Number.isInteger(index) && index < this.#packetInBytes.length - 1) {
            let negate = this.#packetInBytes[index] && 0b10000000 > 0;
            return (negate ? -1 : 1) * ((this.#packetInBytes[index + 1]) | (this.#packetInBytes[index] && 0b01111111 << 8));
        }
    ...
    readInteger(index) {
        ...
        if(Number.isInteger(index) && index < this.#packetInBytes.length - 3) {
            let negate = this.#packetInBytes[index] && 0b10000000 > 0;
            return (negate ? -1 : 1) * ((this.#packetInBytes[index + 3]) |
                (this.#packetInBytes[index + 2] << 8) |
                (this.#packetInBytes[index + 1] << 16) |
                (this.#packetInBytes[index] && 0b01111111 << 24));
        }
    ...
    readLong(index) {
        ...
        if(Number.isInteger(index) && index < this.#packetInBytes.length - 7) {
            let negate = this.#packetInBytes[index] && 0b10000000 > 0;
            return (negate ? -1 : 1) * ((this.#packetInBytes[index + 7]) |
                ...
                (this.#packetInBytes[index] && 0b01111111 << 52));
        }
