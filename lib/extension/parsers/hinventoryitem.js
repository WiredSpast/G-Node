...
const { HStuff } = require("./hflooritem");
const { HFurniType } = require("./hfurnitype");
const { HSpecialType } = require("./hspecialtype");
const util = require("util");

exports.HInventoryItem = class HInventoryItem {
    #itemId;
    #furniType;
    #id;
    #typeId;
    ...
    [util.inspect.custom](depth) {
        ...
        return    `${indent}HInventoryItem {\n`
            ...
            + `${HFurniType.identify(this.#furniType) ? `${indent}  bodyFacing: HFurniType.\x1b[36m${HFurniType.identify(this.#furniType)}\x1b[0m\n` : ''}`
            ...
            + `${this.#furniType === HFurniType.FLOOR ? `${indent}  slotId: ${util.inspect(this.#slotId, {colors: true})}\n` : ''}`
            + `${this.#furniType === HFurniType.FLOOR ? `${indent}  extra: ${util.inspect(this.#extra, {colors: true})}\n` : ''}`
            + `${indent}}`;
    }
    ...
    appendToPacket(packet) {
        ...
        packet.appendInt(this.#itemId)
            .appendString(this.#furniType)
            .appendInt(this.#id)
            .appendInt(this.#typeId)
            .appendInt(this.#category)
            .appendInt(this.#stuffCategory);
        HStuff.appendData(packet, this.#stuffCategory, this.#stuff);
        packet.appendBoolean(this.#isRecyclable)
            .appendBoolean(this.#isTradeable)
            .appendBoolean(this.#isGroupable)
            .appendBoolean(this.#isSellable)
            .appendInt(this.#secondsToExpiration)
            .appendBoolean(this.#hasRentPeriodStarted)
    ...
    static constructPackets(inventoryItems, headerId) {
        ...
        for(let i = 0; i < packetCount; i++) {
            let packet = new HPacket(headerId)
                .appendInt(packetCount)
                .appendInt(i)
                .appendInt(i === packetCount - 1 ? inventoryItems.length % 600 : 600);
    ...
    setFurniType(furniType) {
        ...
        if(!HFurniType.identify(furniType)) {
            throw new Error("HInventoryItem.setFurniType: furnitype must be a value of HFurniType");
        }
    ...
    set furniType(val) {
        if(!HFurniType.identify(val)) {
            throw new Error("HInventoryItem.furniType: must be a value of HFurniType");
        }
