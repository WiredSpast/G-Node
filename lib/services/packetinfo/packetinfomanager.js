...
export class PacketInfoManager {
  ...
  #addMessage = (packetInfo) => {
    ...
    const headerIdToMessage = packetInfo.destination === HDirection.TOCLIENT ? this.#headerIdToMessage_incoming : this.#headerIdToMessage_outgoing;

    const hashToMessage = packetInfo.destination === HDirection.TOCLIENT ? this.#hashToMessage_incoming : this.#hashToMessage_outgoing;

    const nameToMessage = packetInfo.destination === HDirection.TOCLIENT ? this.#nameToMessage_incoming : this.#nameToMessage_outgoing;
  ...
  getAllPacketInfoFromHeaderId (direction, headerId) {
    ...
    const headerIdToMessage = direction === HDirection.TOCLIENT ? this.#headerIdToMessage_incoming : this.#headerIdToMessage_outgoing;
  ...
  getAllPacketInfoFromHash (direction, hash) {
    ...
    const hashToMessage = direction === HDirection.TOCLIENT ? this.#hashToMessage_incoming : this.#hashToMessage_outgoing;
  ...
  getAllPacketInfoFromName (direction, name) {
    ...
    const nameToMessage = direction === HDirection.TOCLIENT ? this.#nameToMessage_incoming : this.#nameToMessage_outgoing;
  ...
  getPacketInfoFromHeaderId (direction, headerId) {
    ...
    const all = this.getAllPacketInfoFromHeaderId(direction, headerId);
  ...
  getPacketInfoFromHash (direction, hash) {
    ...
    const all = this.getAllPacketInfoFromHash(direction, hash);
    return all.length === 0 ? null : all[0];
  }
  ...
  getPacketInfoFromName (direction, name) {
    ...
    const all = this.getAllPacketInfoFromName(direction, name);
    return all.length === 0 ? null : all[0];
  }
  ...
  static readFromPacket (hPacket) {
    const packetInfoList = [];
    const size = hPacket.readInteger();

    for (let i = 0; i < size; i++) {
      const packetInfo = new PacketInfo(...hPacket.read('iSSSbS'));
