...
const { HGender } = require("./hentity");
const { HRelationshipStatus } = require("./hrelationshipstatus");
...
exports.HFriend = class HFriend {
    ...
    constructor(packet, categories = new Map()) {
        ...
        this.#relationshipStatus = packet.readShort();
    }
    ...
    appendToPacket(packet) {
        ...
        packet.appendInt(this.#gender === HGender.Female ? 0 : 1);
        packet.appendBoolean(this.#online);
        packet.appendBoolean(this.#followingAllowed);
        packet.appendString(this.#figure);
        packet.appendInt(this.#categoryId);
        packet.appendString(this.#motto);
        packet.appendString(this.#realName);
        packet.appendString(this.#facebookId);
        packet.appendBoolean(this.#persistedMessageUser);
        packet.appendBoolean(this.#vipMember);
        packet.appendBoolean(this.#pocketHabboUser);
        packet.appendShort(this.#relationshipStatus);
    }
    ...
    static constructFragmentPackets(friends, headerId) {
        ...
        for(let i = 0; i < packetCount; i++) {
            let packet = new HPacket(headerId)
                ...
                .appendInt(i)
                .appendInt(i === packetCount - 1 ? friends.length % 100 : 100);
