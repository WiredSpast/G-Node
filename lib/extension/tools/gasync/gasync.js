...
export class GAsync {
  ...
  #onMessageToServer = (hMessage) => {
    if (this.#packetInfoManager !== undefined) {
      const info = this.#packetInfoManager.getPacketInfoFromHeaderId(HDirection.TOSERVER, hMessage.getPacket().headerId());
      if (info === null) {
        return;
      }
      ...
      this.#awaitingPackets
        ...
        .forEach(p => {
          if (p.blocksHMessage) { hMessage.blocked = true; }
  ...
  #onMessageToClient = (hMessage) => {
    if (this.#packetInfoManager !== undefined) {
      const info = this.#packetInfoManager.getPacketInfoFromHeaderId(HDirection.TOCLIENT, hMessage.getPacket().headerId());
      if (info === null) {
        return;
      }
  ...
  async awaitPacket (...packets) {
    ...
    return new Promise(resolve => {
      const interval = setInterval(() => {
        for (const packet of packets.filter(p => p.ready)) {
